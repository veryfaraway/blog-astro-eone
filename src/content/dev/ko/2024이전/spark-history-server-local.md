...
![Spark History Server](/images/dev/spark-history-ui-2x.png)
...
혹시 `Homebrew`가 설치가 안 되어 있으면 다음 글을 참조하세요 👉 [Homebrew 설치하기](/tools/install-homebrew)
...
```bash
cd /usr/local/Cellar/apache-spark/<version>/libexec/sbin
sh start-history-server.sh --properties-file <path to log.properties>
```
