...
자동 변환 (스크립트):
- [ ] `{% alert %}` / `{{< alert >}}` → `<Alert>` MDX 컴포넌트 (105개)
- [ ] `{% youtube %}` / `{{< youtube >}}` → `<iframe>` 임베드 (~20개)
- [ ] `{{< mermaid >}}` → ` ```mermaid ` 코드 펜스 + rehype-mermaid (11개)
- [ ] `{{< ref "..." >}}` → 슬러그 기반 상대 링크 (3개)
- [ ] `{% raw %}` / `{% endraw %}` → 제거 (2개)

수동 변환 (이전 완료 후):
- [ ] `{{< clock24 >}}` (2개) — 시계 컴포넌트 직접 구현 후 교체
- [ ] `{{< chartjs >}}` (3개) — Chart.js 기반 React 컴포넌트 직접 구현 후 교체
- [ ] `{% personInline %}` (2개) — 인물 카드 컴포넌트 또는 일반 텍스트로 처리
- [ ] `{% adsense %}` (2개) — Astro AdSense 컴포넌트로 교체
